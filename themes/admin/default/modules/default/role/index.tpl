		...
		{foreach from=$roles item=role key=index}
			<div class="grid_12{if ($index + 1) mod 2 eq 0} even{else} odd{/if} faux-tr  alpha omega">
				<div class="grid_4 alpha">
					<p>{if $role.shortname ne "guest"}<a href="{url}/default/role/edit/id/{$role.id}{/url}">{$role.shortname}</a>{else}{$role.shortname}{/if}</p>
				</div>
				<div class="grid_4">
					<p><a href="{url}/default/resource/edit/id/{$role.id}{/url}">{t}Resource Permissions{/t}</a></p>
				</div>
