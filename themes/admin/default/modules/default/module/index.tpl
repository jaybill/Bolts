	...
	{foreach from=$modules item=module key=index}
		<div class="grid_16{if ($index + 1) mod 2 eq 0} even{else} odd{/if} faux-tr">
			...
			<div class="grid_2 omega">
				{if $module.available}	
					<a class="icon module-install" href="{url}/default/module/index/id/{$module.id}/perform/install{/url}" title="{t}Install{/t}">{t}Install{/t}</a>		
				{elseif $module.is_enabled eq 1}		
					<a class="icon module-disable" href="{url}/default/module/index/id/{$module.id}/perform/disable{/url}" title="{t}Disable{/t}">{t}Disable{/t}</a>
					<a class="icon module-uninstall" href="{url}/default/module/uninstall/id/{$module.id}{/url}" title="{t}Uninstall{/t}">{t}Uninstall{/t}</a>
				{elseif $module.is_enabled eq 0}
					<a class="icon module-enable" href="{url}/default/module/index/id/{$module.id}/perform/enable{/url}" title="{t}Enable{/t}">{t}Enable{/t}</a>
					<a class="icon module-uninstall" href="{url}/default/module/uninstall/id/{$module.id}{/url}" title="{t}Uninstall{/t}">{t}Uninstall{/t}</a>
				{/if}
			</div>
		</div>
