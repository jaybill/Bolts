...
<div class="grid_4 sidenav">
	<h3>{t}Options{/t}</h3>
	<ul>
		<li><a href="{url}/bolts/role/index{/url}">{t}Back to Roles{/t}</a></li>
		{if isset($role.id)}
			<li><a href="{url}/bolts/resource/edit/id/{$role.id}{/url}">{t}Resources{/t}</a></li>
			<li><a href="{url}/bolts/navigation/editrole/id/{$role.id}{/url}">{t}Navigation{/t}</a></li>
			<li><a href="{url}/bolts/role/delete/id/{$role.id}{/url}">{t}Delete Role{/t}</a></li>
		{/if}
	<ul>
</div>
<div class="grid_12">
	<form action="{url}/bolts/role/edit{/url}" method="post">
		<p>
