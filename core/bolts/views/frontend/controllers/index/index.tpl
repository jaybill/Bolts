		...
		{if count($recently_registered) >= 0}
			...
			<p class="thumbs">
				{foreach from=$recently_registered item=user key=index}
					<a href="{url}/bolts/user/profile/bolts/username/{$user.username}{/url}"><img
...
<div class="grid_4 omega">
	{if $isLoggedIn ne true}			
		<form action="{url}/bolts/auth/login{/url}" method="post" id="login" class="sidebar-form">				
			<p>	
	...
	<p>Are you short on friends? Are you just short? We can help! <a href="{url}/bolts/user/register{/url}">Register now</a> and you can be cranking out friends in no time!</p>
</div>
